...
in
{
  # Only enable i3 if X11 is available and not in CI
  xsession.windowManager.i3 = lib.mkIf (!isCI && hasX11) {
    enable = true;
    ...
    config = {
      ...
      # Startup applications (only set wallpaper, no missing files)
      startup = [
